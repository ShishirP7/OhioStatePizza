...
export default function MyCart() {
  ...
  const calculateTotal = () => {
    ...
    cartItems.forEach((item) => {
      ...
      if (custom.selectedSize) {
        const opt = item.options?.sizes?.find((o) => o.label === custom.selectedSize);
      ...
      if (item.category === "Pizzas" && custom.selectedCrust) {
        const opt = item.options?.crusts?.find((o) => o.label === custom.selectedCrust);
        if (opt) itemTotal += opt.priceModifier;
      }
  ...
  return (
    <div className="min-h-screen bg-gray-100 text-black py-8 px-4">
      <h1 className="text-3xl font-bold text-center mb-6">Customize My Orders</h1>
                      ...
                      <div className="flex gap-2 flex-wrap">
                        {item.options.veggies.map((opt) => (
                          <button
                            ...
                            onClick={() =>
                              toggleOption(item.id, "selectedVeggies", opt.label)
                            }
                          >
                            {opt.label} (+${opt.priceModifier})
                          </button>
